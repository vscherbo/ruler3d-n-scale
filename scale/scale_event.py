...
class HX711:
    ...
    def request_lines(self):
        ...
        config_data = gpiod.LineSettings(direction=Direction.INPUT)
        config_data.set_edge_detection("both")
        ...
        self.line_request = self.chip.request_lines(
            ...
            output_values={self.clock_line: 0}
        )
    ...
    def read_raw(self) -> int:
        ...
        """
        with self.line_request.read_edge_events(self.data_line) as event_buffer:
            while not any(event_buffer):
                pass

            value = 0
            for _ in range(24 + self.gain_bits):
                self.line_request.set_value(self.clock_line, 1)
                time.sleep(0.000001)
                if any(event_buffer):
                    value = (value << 1) | self.line_request.get_value(self.data_line)
                self.line_request.set_value(self.clock_line, 0)
                time.sleep(0.000001)
